/**
* @Author 王姝昕
* @Date 2023年05月27日 14:42
* @Description 维修信息管理页面
* 
*/
<template>
  <cli-layout cur-index="5-2">
    <div >
      <el-card class="card">
      <!--导航栏设计-->
      <el-form :inline="true" class="demo-form-inline">
        ...
        <el-form-item>
          <el-button class="el-icon-circle-plus-outline" type="text" @click="emptyMaintenanceData(); dialogVisible = true">添加</el-button>
        </el-form-item>
      ...
      <el-table :data="tableData" highlight-current-row border style="width: 100%">
        ...
        <el-table-column label="维修时间">
          <template slot-scope="scope">
            <span>{{ scope.row.maintenanceTime }}</span>
          </template>
        </el-table-column>

        <el-table-column label="维修类型">
          <template slot-scope="scope">
            <span>{{ scope.row.maintenanceType }}</span>
          </template>
        </el-table-column>
      ...
      <el-form :model="ruleForm" :ref="ruleForm" label-width="70px" class="demo-ruleForm" size="medium">
        <el-dialog title="添加" :append-to-body='true' :visible.sync="dialogVisible" width="50%" :before-close="handleClose">
          ...
          <el-form-item label="维修时间" prop="maintenanceTime">
            <el-date-picker v-model="ruleForm.maintenanceTime" type="datetime" placeholder="选择日期时间" value-format="yyyy-MM-dd HH:mm:ss">
            </el-date-picker>

          </el-form-item>
       
          <el-form-item label="维修类型" prop="maintenanceType">
            <el-select v-model="ruleForm.maintenanceType" placeholder="请选择维修类型">
              <el-option label="轮胎更换" value="轮胎更换"></el-option>
              <el-option label="刹车维修" value="刹车维修"></el-option>
              <el-option label="链条维修" value="链条维修"></el-option>
              <el-option label="坐垫维修" value="坐垫维修"></el-option>
              <el-option label="其它维修" value="其它维修"></el-option>
            </el-select>
          </el-form-item>
          
          <el-form-item label="维修状态" prop="maintenanceStatus">
            <el-select v-model="ruleForm.maintenanceStatus" placeholder="请选择维修状态">
              <el-option label="待维修" value="待维修"></el-option>
              <el-option label="维修中" value="维修中"></el-option>
              <el-option label="已完成维修" value="已完成维修"></el-option>
            </el-select>
          </el-form-item>
            ...
            <span slot="footer" class="dialog-footer">
              ...
              <el-button @click="addMaintenance()" type="primary" size="medium">确 定</el-button>
            </span>
        </el-dialog>
      </el-form>
      <!--添加对话框结束-->
      ...
      <el-form :model="ruleForm" :ref="ruleForm" label-width="70px" class="demo-ruleForm" size="medium">
        <el-dialog title="修改" :append-to-body='true' :visible.sync="dialogUpdate" width="50%" :before-close="handleClose">
          ...
          <span slot="footer" class="dialog-footer">
              <el-button @click="cancel()" size="medium">取 消</el-button>
              <el-button @click="updateMaintenance()" type="primary" size="medium">确 定</el-button>
            </span>
        </el-dialog>
      </el-form>
      <!--修改对话框结束-->
...
import 'default-passive-events';
  ...
  created () {
    const _this = this
    axios({
      url: 'http://localhost:8888/Maintenance-service/maintenance/showMaintenance',
      method: 'get',
      headers: {
      ...
      if (response.data === 'fail') {
        ...
        _this.tableData = response.data.maintenanceList
      }
    })
    ...
    handleDelete (index, row) { // -----------------------------------------删除操作
      ...
      this.$confirm('删除操作, 是否继续?', '提示', {
            ...
            axios({
              method: 'post',
              url: 'http://localhost:8888/Maintenance-service/maintenance/deleteMaintenance',
              data: postData,
              headers: {
                token: Cookies.get('token')
              },
    ...
    cancel () {
      ...
      this.emptyMaintenanceData()
    },
    /* 表格清空函数 */
    emptyMaintenanceData () {
      this.ruleForm = {
        id: '',
        bikeNumber: '',
        maintenanceTime: '',
    ...
    addMaintenance () { // ----------------------------------------------添加操作
      const postData = qs.stringify({
        ...
        maintenanceTime: this.ruleForm.maintenanceTime,
        maintenanceType: this.ruleForm.maintenanceType,
        maintenanceStatus: this.ruleForm.maintenanceStatus
      })
 
      axios({
        method: 'post',
        url: 'http://localhost:8888/Maintenance-service/maintenance/addMaintenance',
        data: postData,
        headers: {
          token: Cookies.get('token')
        },
    ...
    updateMaintenance () { // -----------------------------------------更新操作
      ...
      axios({
        method: 'post',
        url: 'http://localhost:8888/Maintenance-service/maintenance/updateMaintenance',
        data: postData,
        headers: {
          token: Cookies.get('token')
        },
    ...
    onSearch () { // --------------------------------------------------查询操作
      ...
      axios({
        method: 'post',
        url: 'http://localhost:8888/Maintenance-service/maintenance/findMaintenanceByNum',
        data: postData,
        headers: {
          token: Cookies.get('token')
        },
      }).then(response => {
        this.tableData = response.data.maintenanceList
